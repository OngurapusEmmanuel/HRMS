...
A real migration history is committed under `prisma/migrations/` (currently `init` → `feature_completeness` → `production_readiness_indexes`), applied via `prisma migrate deploy` against a live Neon Postgres instance — CI's `migrate deploy` path is the one actually exercised, not the `db push` fallback. Full step-by-step procedure for setting up a fresh environment: [`docs/production-migration-runbook.md`](./docs/production-migration-runbook.md).
...
- **Rate limiting** — login (`lib/auth.ts`'s `authorize()`), `/api/auth/forgot-password`, and `/api/auth/reset-password` are all limited via `lib/rate-limit.ts`, an in-memory limiter explicitly documented as single-instance-only — swap for Upstash Redis or similar before running multiple instances/serverless.
- **Env var validation** — `lib/env.ts` validates `process.env` with zod at import time (imported from `lib/db.ts`, so it runs on first server-side import), so a missing/malformed env var fails loudly at boot instead of surfacing as a confusing error mid-request.
- **Error boundaries** — `app/error.tsx`/`app/global-error.tsx`/`app/not-found.tsx` and a `(dashboard)/error.tsx` scoped boundary (keeps the sidebar/topbar chrome visible) replace Next's bare default error screen with something matching the rest of the app's design system.
- **Security headers** — `next.config.js` sets CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy, and HSTS on every response; Server Actions' `allowedOrigins` derives from `NEXTAUTH_URL` instead of a wildcard.
...
- **`STORAGE_PROVIDER=local` is incompatible with a Vercel deployment.** `vercel.json` implies Vercel as a deployment target, but Vercel's filesystem is ephemeral/read-only outside `/tmp` at runtime — local-disk document uploads would silently fail to persist across deployments/scaling. Set `STORAGE_PROVIDER=s3` (with S3/R2/GCS credentials) before deploying to Vercel; local storage is only safe for the Docker Compose path, where the upload volume is a real persistent mount.
- No structured logging or error-tracking integration (e.g. Sentry) — errors currently go to `console.error`/stdout only, which on Vercel means they're visible in function logs with no aggregation, alerting, or stack-trace grouping across deploys.
- `docker-compose.yml`/`Dockerfile` (a full local-stack option, Postgres included) and `vercel.json` (Vercel Cron for the leave-reminders job) both still exist — pick one as the actual deployment target; running on Vercel needs the S3 storage fix above, running via Docker needs its own cron mechanism for `/api/cron/leave-reminders` (system cron, `node-cron`) since Vercel Cron doesn't apply there.
